  ...
  export default function ContactsPage() {
    ...
    // Handle search and filters - applies filters and resets to page 1 only when filters/search actually change
    useEffect(() => {
      ...
      if (showViewedOnly && limitReached) {
        ...
        filtered = filtered.filter((contact) => viewedIds.has(contact.id));
      }
      ...
      setCurrentPage(1);
    }, [searchTerm, filters, showViewedOnly]);
    ...
    // Handle opening contact details modal (without changing current page)
    const handleViewContact = (contact: Contact) => {
      ...
      // Keep current page - don't reset to page 1
              ...
              <table className={styles.table}>
                ...
                <tbody>
                  {paginatedContacts.map((contact, idx) => {
                    const viewedIds = user ? getViewedContactIds(user.id) : new Set();
